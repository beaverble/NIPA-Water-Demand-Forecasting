...
def train_test_time():
    ...
    test_start = datetime.datetime.strftime(test_end - relativedelta(days=3), '%Y-%m-%d %H:%M:%S')
    train_end = datetime.datetime.strftime(test_end - relativedelta(days=3), '%Y-%m-%d %H:%M:%S')
    train_start =datetime.datetime.strftime(test_end - relativedelta(days=10), '%Y-%m-%d %H:%M:%S')
...
def main(dataframe):
    ...
    train_day = predict_usage_short.day_cal(train_data)
    test_day = predict_usage_short.day_cal(test_data)
    ...
    path_checkpoint = '2022_short_normal.h5'
    ...
    model = load_model('2022_test_short_m.h5')
    model.fit(final_x_train, final_y_train, validation_data=(final_x_test, final_y_test), batch_size=16,
              epochs=1000, verbose=2, callbacks=[early_stopping, callback_checkpoint])
    model.save('2022_test_short_m.h5')
    print("test finish")

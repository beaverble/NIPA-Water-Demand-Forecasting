...
def remove_cusnum(dataset, cusnum_list):
    ...
    for i in range(len(cusnum_list)):
        if len(dataset[dataset["CUSNUM"] == cusnum_list[i]]) != 3:
        ...
        if i < (len(dataset.index) - 1):
            if not dataset.index[i][0] == dataset.index[i+1][0]:
                ...
                scaled_usage_array = np.append(scaled_usage_array,
                                             globals()['scaler_usage_{}'.format(count)].fit_transform((scaled_usage.values).reshape(-1,1)),axis=0)
                k = i+1
...
def main(data):
    ...
    short_model = load_model('2022_test_short_m.h5')
    pred_y = short_model.predict(final_x_test)
    ...
    for i in range(len(pred_y)):
        result.append([{"date": ymd, "usage": str(pred_y[i][0])}])
...
if __name__ == '__main__':
    #data = pd.read_csv("2022-10-12_short.csv")
    main()
    #make_log()
    #add_log()
